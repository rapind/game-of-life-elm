...
Elm.StaticGrid = Elm.StaticGrid || {};
Elm.StaticGrid.make = function (_elm) {
   "use strict";
   _elm.StaticGrid = _elm.StaticGrid || {};
   if (_elm.StaticGrid.values) return _elm.StaticGrid.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Color = Elm.Color.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var _p0 = {ctor: "_Tuple2",_0: 35,_1: 35};
   var columns = _p0._0;
   var rows = _p0._1;
   var generateGrid = A2($List.repeat,rows,A2($List.repeat,columns,true));
   var cellSize = 5;
   var renderCell = function (on) {
      return A2($Graphics$Element.color,on ? A3($Color.rgb,0,0,0) : A3($Color.rgb,255,255,255),A2($Graphics$Element.spacer,cellSize,cellSize));
   };
   var renderRow = function (row) {    return A2($List.map,renderCell,row);};
   var renderGrid = function (grid) {
      return A4($Graphics$Element.container,
      cellSize * columns,
      cellSize * rows,
      $Graphics$Element.topLeft,
      A2($Graphics$Element.flow,$Graphics$Element.down,A2($List.map,$Graphics$Element.flow($Graphics$Element.right),A2($List.map,renderRow,grid))));
   };
   return _elm.StaticGrid.values = {_op: _op,generateGrid: generateGrid,renderGrid: renderGrid};
};
Elm.Main = Elm.Main || {};
Elm.Main.make = function (_elm) {
   ...
   $Result = Elm.Result.make(_elm),
   $Signal = Elm.Signal.make(_elm),
   $StaticGrid = Elm.StaticGrid.make(_elm);
   var _op = {};
   var main = $StaticGrid.renderGrid($StaticGrid.generateGrid);
